...
import { createTrial } from './api/createTrial.js';
...
app.use((req, res, next) => {
  res.header('Access-Control-Allow-Origin', '*');            // <-- change this if you want stricter origins
...
// ----------  Stripe endpoints ----------
app.post('/v1/api/create-trial', createTrial);               // ← Stripe checkout
app.post('/v1/api/stripe-webhook', stripeWebhook);          // ← incoming webhook
...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    res
      ...
      .json({ error: { message: error.message || 'Internal server error', type: error.type || 'api_error' } });
  }
});
...
// ----------  Dashboard (same page that shows optimizer stats) ----------
...
<head>
  ...
  <style>
    body{font-family:system-ui,sans-serif;margin:0 auto;padding:20px;max-width:1200px;}
    ...
    .badge{padding:4px 8px;border-radius:4px;font-size:.8em;}
...
<body>
  ...
  <div class="stats">
    ...
    <div class="card"><h3>Total Cost</h3><div class="value">$${stats.totalCost}</div></div>
    <div class="card"><h3>Total Tokens</h3><div class="value">${stats.totalTokens.toLocaleString()}</div></div>
    <div class="card"><h3>Cache Hit Rate</h3><div class="value">${stats.cacheHitRate}</div></div>
  </div>
  <div class="recent">
